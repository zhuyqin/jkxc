...
//	}
	...
	public Result<?> getUserPermissionByToken() {
		...
		try {
			...
			SysPermission indexMenu = null;
			// 查找是否已有首页菜单
			for (SysPermission permission : metaList) {
				if ("首页".equals(permission.getName())) {
					indexMenu = permission;
					break;
				}
			}
			// 如果没有首页菜单，从数据库获取
			if (indexMenu == null) {
				indexMenu = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName,"首页")).get(0);
				metaList.add(0, indexMenu);
			}
			//update-begin--Author:liusq  Date:20210624  for:自定义首页地址LOWCOD-1578
			// 无论首页菜单是否存在，都根据角色更新component和url
			...
			if(roles.size()>0){
				for (String code:roles) {
					...
					if(StringUtils.isNotBlank(componentUrl)){
						...
						// 同时更新url字段，使其与component对应（前端path字段对应后端的url字段）
						// component格式：dashboard/Dashboard 或 dashboard/Workbench
						// 转换为url格式：/dashboard/Dashboard 或 /dashboard/Workbench
						String urlPath = "/" + componentUrl;
						indexMenu.setUrl(urlPath);
						break;
					}
				}
			}
