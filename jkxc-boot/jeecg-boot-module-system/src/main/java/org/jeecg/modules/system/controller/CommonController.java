...
public class CommonController {
    ...
    /**
     * 本地：local minio：minio 阿里：alioss FTP：ftp
     */
     ...
     * FTP配置
     */
    @Value(value="${jeecg.ftp.host:}")
    private String ftpHost;
    
    @Value(value="${jeecg.ftp.port:21}")
    private int ftpPort;
    
    @Value(value="${jeecg.ftp.username:}")
    private String ftpUsername;
    
    @Value(value="${jeecg.ftp.password:}")
    private String ftpPassword;
    
    @Value(value="${jeecg.ftp.basePath:}")
    ...
    public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(CommonConstant.UPLOAD_TYPE_LOCAL.equals(uploadType)){
            ...
            */
            }else if(CommonConstant.UPLOAD_TYPE_FTP.equals(uploadType)){
                // FTP上传
                savePath = FtpUtil.upload(file, bizPath, ftpHost, ftpPort, ftpUsername, ftpPassword, ftpBasePath);
        }else{
...
//	}
    ...
    public void view(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 检查是否是错误消息（不应该作为文件路径）
        if (imgPath.startsWith("上传失败") || imgPath.contains("创建FTP目录失败") || imgPath.contains("无法连接")) {
            response.setStatus(404);
            log.warn("尝试访问错误消息作为文件路径：{}", imgPath);
            return;
        }
        ...
        try {
            ...
            // 根据上传类型处理文件访问
            if (CommonConstant.UPLOAD_TYPE_FTP.equals(uploadType)) {
                // FTP模式：从FTP服务器下载文件
                try {
                    // 设置响应头
                    ...
                    FtpUtil.download(imgPath, ftpBasePath, outputStream, ftpHost, ftpPort, ftpUsername, ftpPassword);
                    response.flushBuffer();
                } catch (Exception e) {
                    log.error("从FTP下载文件失败：" + imgPath + "，错误：" + e.getMessage());
                    response.setStatus(404);
                }
            } else {
                // 本地模式：从本地文件系统读取
