...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	public List<String> loadDictItem(String ids, boolean delNotExist) {
		...
		for (String raw : idArray) {
			if (raw != null && oConvertUtils.isNotEmpty(raw.trim())) {
				idList.add(raw.trim());
			}
		}
		if (idList.isEmpty()) {
		...
		query.in(SysCategory::getId, idList);
		List<SysCategory> list = super.list(query);
		// 必须与请求 ids 顺序一一对应，否则前端批量翻译会错行（例如线索名称显示成另一条分类）
		List<String> textList = new ArrayList<>(idList.size());
		for (String id : idList) {
			String name = list.stream()
					.filter(i -> id.equals(i.getId()))
					.map(SysCategory::getName)
					.findFirst()
					.orElse(null);
			if (name != null) {
				textList.add(name);
