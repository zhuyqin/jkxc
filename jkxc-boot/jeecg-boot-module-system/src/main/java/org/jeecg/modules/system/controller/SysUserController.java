...
public class SysUserController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
		...
		// 处理status参数：如果包含逗号，则使用IN查询
		String statusParam = req.getParameter("status");
		String statusInParam = req.getParameter("status_in");
		if (oConvertUtils.isNotEmpty(statusInParam)) {
			// 如果使用status_in参数，先设置为null避免类型转换错误
			user.setStatus(null);
		} else if (oConvertUtils.isNotEmpty(statusParam) && statusParam.contains(",")) {
			// 如果status参数包含逗号，先设置为null避免类型转换错误
			user.setStatus(null);
		}
		
		QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(user, req.getParameterMap());
		
		// 手动处理status_in或包含逗号的status参数
		if (oConvertUtils.isNotEmpty(statusInParam)) {
			String[] statusArray = statusInParam.split(",");
			List<Integer> statusList = new ArrayList<>();
			for (String s : statusArray) {
				if (oConvertUtils.isNotEmpty(s.trim())) {
					try {
    ...
    public Result<IPage<SysUser>> queryUserComponentData(
        ...
        IPage<SysUser> pageList = sysUserDepartService.queryDepartUserPageList(departId, username, realname, roleId, pageSize, pageNo);
