(function (global, factory) {
  ...
  function withToSQL(withExpr) {
    ...
    const withExprStr = withExpr.map(cte => {
      ...
      const expr = (0, _util.commonOptionConnector)(stmt.type === 'values' ? 'VALUES' : '', _expr.exprToSQL, stmt);
      return `${name.type === 'default' ? (0, _util.identifierToSql)(name.value) : (0, _util.literalToSQL)(name)}${column} AS (${expr})`;
    }).join(', ');
