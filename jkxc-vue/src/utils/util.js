...
/**
 * 处理输入框的值：去除前后空白符，过滤掉JInput组件的**和前后的*
 * JInput组件type="like"时，空值会返回"**"，有值时会返回"*value*"
 * 对于普通输入框，会去除前后的空白符（空格、制表符、换行符等）
 * @param value 输入框的值
 * @returns {string} 处理后的值，如果为空则返回空字符串
 */
export function processJInputValue(value) {
  // 如果不是字符串，直接返回原值
  ...
  // 如果为空或者是JInput的空值标记，返回空字符串
  if (!trimmedValue || trimmedValue === '**') {
    return ''
  }
  
  // 如果是JInput的like类型（前后都有*），去掉前后的*并再次trim
...
/**
 * 处理查询参数对象，去除所有字符串值的前后空白符，过滤掉JInput组件的**值
 * 这个函数会自动处理所有input输入框的查询参数，去除用户不小心输入的前后空格
 * @param params 查询参数对象
 * @returns {*} 处理后的查询参数对象
 */
export function processQueryParams(params) {
  ...
  for (const key in processed) {
    if (processed.hasOwnProperty(key)) {
      ...
      // 处理字符串类型的值：去除前后空白符
      if (typeof value === 'string') {
        const processedValue = processJInputValue(value)
        if (processedValue === '') {
          // 如果处理后为空，删除该参数
...
export function generateIndexRouter(data) {
  // 优先使用已保存的首页路径，否则从菜单数据中查找
  let indexPath = Vue.ls.get(INDEX_MAIN_PAGE_PATH) || '/dashboard/analysis' // 默认路径
  
  // 如果localStorage中没有，尝试从菜单数据中查找（优先根据component生成路径）
  if (!Vue.ls.get(INDEX_MAIN_PAGE_PATH) && data && data.length > 0) {
    function findIndexMenu(menuList) {
      for (let menu of menuList) {
        if (menu.meta && menu.meta.title === '首页') {
          // 优先根据component生成路径（因为后端可能只更新了component，没有更新path）
          if (menu.component) {
            if (menu.component === 'dashboard/Dashboard') {
              indexPath = '/dashboard/Dashboard'
            } else if (menu.component === 'dashboard/Workbench') {
              indexPath = '/dashboard/Workbench'
            } else if (menu.path) {
              indexPath = menu.path
            }
          } else if (menu.path) {
