<template>
  <div class="customer-value-analysis">
    <!-- 只在初始加载时显示全局loading -->
    <a-spin :spinning="loading && !statistics.totalCount" tip="正在加载数据...">
    <a-card :bordered="false">
      ...
      <a-row :gutter="16">
        ...
        <a-col :span="24" style="margin-bottom: 16px">
          <a-card title="月度价值趋势" :bordered="false" class="chart-card">
        ...
        <!-- 客户价值排名 -->
        <a-col :span="12" style="margin-bottom: 16px">
          <a-card title="客户价值排名（Top 10）" :bordered="false" class="chart-card">
            <div ref="customerValueRankChart" style="width: 100%; height: 350px"></div>
          </a-card>
        </a-col>

        <!-- 每日新签趋势 -->
        <a-col :span="24" style="margin-bottom: 16px">
          <a-card title="每日价值趋势" :bordered="false" class="chart-card">
        ...
        <!-- 业务人员月度明细 -->
        <a-col :span="24">
          <a-card title="业务人员价值月度明细" :bordered="false" class="table-card">
            <template v-if="tableLoading" slot="extra">
              <a-spin size="small" />
    ...
    >
      ...
      >
        ...
        <template slot="lossTime" slot-scope="text">
          {{ formatDateTime(text) }}
        </template>
        <template slot="valueLevel" slot-scope="text, record">
          <a-tag :color="record.valueLevel === 1 ? 'red' : 'default'" style="margin: 0">
            {{ record.valueLevel === 1 ? '高价值' : '普通' }}
          </a-tag>
        </template>
      </a-table>
    </j-modal>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      charts: {
        monthlyTrend: null,
        businessType: null,
        customerValueRank: null,
        dailyTrend: null,
      },
      salesmanMonthlyData: [],
      salesmanMonthlyColumns: [],
      loading: false,
      tableLoading: false, // 表格单独的loading状态
      isLoadingData: false, // 防止重复请求
      ...
      orderListColumns: [
        ...
        },
        {
          ...
          key: 'lossTime',
          width: 180,
          scopedSlots: { customRender: 'lossTime' },
        },
        {
          title: '创建时间',
        ...
        },
        {
          ...
          key: 'valueLevel',
          width: 100,
          scopedSlots: { customRender: 'valueLevel' },
        },
      ],
    }
  },
  ...
  methods: {
    initCharts() {
      ...
      // 初始化客户价值排名图
      this.charts.customerValueRank = echarts.init(this.$refs.customerValueRankChart)
      // 初始化每日趋势图
    ...
    handleShowOrderList(salesman, month, count) {
      ...
      this.orderListModal.title = `${salesman} - ${month}月客户列表（共${count}单）`
    ...
    async loadOrderList(salesman, month) {
      ...
      try {
        ...
        const res = await getAction('/customer/ghCustomer/getValueSalesmanMonthlyCustomers', params)
    ...
    async loadData() {
      ...
      try {
        ...
        console.log('开始加载客户价值数据，参数:', params)
        
        // 第一步：先加载统计数据（最快）
        const statsRes = await getAction('/customer/ghCustomer/getValueStatistics', params)
        
        if (statsRes.success && statsRes.result) {
          // 立即显示统计卡片
          this.statistics = statsRes.result.statistics || this.statistics
          this.loading = false // 关闭全局loading，让用户看到统计数据
          
          // 第二步：加载图表数据（分批渲染）
          this.chartData = {
            monthlyTrend: statsRes.result.monthlyTrend || [],
            businessType: statsRes.result.businessType || [],
            customerValueRank: statsRes.result.customerValueRank || [],
          ...
          setTimeout(() => {
            ...
            this.updateCustomerValueRankChart()
          }, 100)
        ...
        // 第三步：异步加载业务人员明细表格（最慢，但不阻塞其他内容）
        const detailRes = await getAction('/customer/ghCustomer/getValueSalesmanMonthlyDetail', params)
        
        if (detailRes.success && detailRes.result) {
          this.salesmanMonthlyData = detailRes.result || []
          console.log('业务人员月度明细数据加载完成，共', this.salesmanMonthlyData.length, '条')
        } else {
    ...
    updateCustomerValueRankChart() {
      if (!this.charts.customerValueRank) return
      const data = this.chartData.customerValueRank || []
      if (!data || data.length === 0) {
        return
      }
      const option = {
        tooltip: {
          ...
          formatter: (params) => {
            ...
            return `${param.name}<br/>客户价值: ¥${param.value.toLocaleString()}`
          },
        },
        grid: {
          left: '15%',
        ...
        yAxis: {
          type: 'category',
          data: data.map((item) => item.customerName || '未命名客户'),
          inverse: true,
          axisLine: {
            lineStyle: {
              color: '#d9d9d9',
      ...
      this.charts.customerValueRank.setOption(option)
    },
