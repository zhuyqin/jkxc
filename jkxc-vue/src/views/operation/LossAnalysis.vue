  ...
  <div class="loss-analysis">
    ...
    <a-card :bordered="false">
      ...
      <a-row :gutter="16">
        ...
        <a-col :span="24" style="margin-bottom: 16px">
          <a-card title="月度流失趋势" :bordered="false" class="chart-card">
        ...
        <!-- 业务员排名 -->
        <a-col :span="12" style="margin-bottom: 16px">
          <a-card title="业务员流失排名（Top 10）" :bordered="false" class="chart-card">
        ...
        <!-- 每日新签趋势 -->
        <a-col :span="24" style="margin-bottom: 16px">
          <a-card title="每日流失趋势" :bordered="false" class="chart-card">
        ...
        <!-- 业务人员月度明细 -->
        <a-col :span="24">
          <a-card title="业务人员流失月度明细" :bordered="false" class="table-card">
            <a-table
    ...
    >
      ...
      >
        ...
        <template slot="lossFlag" slot-scope="text, record">
          <a-tag :color="record.lossFlag === 1 ? 'red' : 'default'" style="margin: 0">
            {{ record.lossFlag === 1 ? '已流失' : '正常' }}
          </a-tag>
        </template>
      </a-table>
    </j-modal>
...
export default {
    ...
    return {
      ...
      orderListColumns: [
        ...
        },
        {
          ...
          key: 'lossFlag',
          width: 100,
          scopedSlots: { customRender: 'lossFlag' },
        },
      ],
    }
  },
  ...
  methods: {
    ...
    handleShowOrderList(salesman, month, count) {
      ...
      this.orderListModal.title = `${salesman} - ${month}月流失列表（共${count}单）`
    ...
    async loadOrderList(salesman, month) {
      ...
      try {
        ...
        const res = await getAction('/order/accountingContract/getLossSalesmanMonthlyContracts', params)
    ...
    async loadData() {
      ...
      try {
        ...
        const [statsRes, detailRes] = await Promise.all([
          getAction('/order/accountingContract/getLossStatistics', params),
          getAction('/order/accountingContract/getLossSalesmanMonthlyDetail', params),
        ])
