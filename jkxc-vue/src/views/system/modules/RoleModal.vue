      ...
      <a-form-model ref="form"  v-bind="layout"  :model="model" :rules="validatorRules">
        ...
        <a-form-model-item label="上级角色" prop="parentId">
          <a-tree-select
            ...
            placeholder="无（根节点）"
            :disabled="roleDisabled && !!model.id">
          </a-tree-select>
        </a-form-model-item>
        <a-form-model-item label="角色类型" required prop="roleType">
          <a-radio-group v-model="model.roleType">
            <a-radio :value="1">操作角色</a-radio>
            <a-radio :value="2">数据角色</a-radio>
          </a-radio-group>
        </a-form-model-item>
        <a-form-model-item label="角色排序" prop="roleOrder">
          <a-input-number v-model="model.roleOrder" :min="0" style="width:100%" placeholder="数字越小越靠前"/>
        </a-form-model-item>
...
<script>
  import {addRole,editRole,duplicateCheck, queryRoleTreeList } from '@/api/api'
  export default {
    name: "RoleModal",
    data () {
      return {
